...
Indeed, the soft k-means model used here exhibits multimodality in the posterior distribution.
...
The leftward point at `x = -0.687` shows a distribution of higher soft Z values for cluster 2 and which is clearly distinct from the distribution of soft Z values for cluster 1.
...
However, for the middle point at `x = 1.4`, the two soft Z-distributions overlap strongly.
...
If we summarize the soft-Z distributions by their median value, soft k-means clustering can be converted to a "hard" k-means by assigning each point to its cluster of highest median probability.
